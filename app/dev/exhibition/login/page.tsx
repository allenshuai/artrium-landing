...
export default function DevExhibitionLogin() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch("/api/dev/exhibition/auth", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ password }),
      });
      if (res.ok) {
        window.location.href = "/dev/exhibition";
        return;
      }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-4">
      ...
      >
        <h1 className="text-xl font-semibold">Exhibition Dev</h1>
        <p className="mt-1 text-sm text-[#3F3A36]/60">Internal tool. Enter the dev password.</p>
        <input
          type="password"
          autoFocus
          value={password}
